...
#include "@VMODROOT/shaders/uniforms.h" # # It should be generated with `v shader uniforms.glsl`

fn C.uniforms_shader_desc(gfx.Backend) &gfx.ShaderDesc // uniforms name comes from the name specified in the shader in @program
...
fn main() {
	...
	sapp.run(
		...
		high_dpi:          true
		window_title:      c'Shaders-uniforms'
	)
}
...
fn init(mut app App) {
	...
	vertices := [Vertex{-0.5, -0.5, 0.0}, Vertex{0.5, -0.5, 0.0},
		Vertex{0.0, 0.5, 0.0}]
	size := u32(vertices.len) * sizeof(Vertex)
	...
	shader := gfx.make_shader(C.uniforms_shader_desc(gfx.query_backend()))
...
fn frame(mut app App) {
	...
	uniform := [f32(0.0), f32(math.sin((time.now() - start).seconds())) / 2.0 + 0.5, 0.0, 1.0]
	data := gfx.Range{
		ptr:  uniform.data
		size: sizeof(f32) * u32(uniform.len)
	}
	gfx.apply_uniforms(.fs, C.SLOT_fs_params, data)

	gfx.draw(vs_in_position_loc, 3, 1)
	gfx.end_pass()
	gfx.commit()
}
